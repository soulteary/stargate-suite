...
本目录**仅保留说明**。实际 Traefik compose 在 `build/traefik/`（三合一）与 `build/traefik-herald/`、`build/traefik-warden/`、`build/traefik-stargate/`（三分开），由 canonical 生成。使用前请先执行原生 CLI `make gen`，否则 `build/traefik/` 不存在。
